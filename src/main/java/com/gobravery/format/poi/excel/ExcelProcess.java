...
public class ExcelProcess {
	...
	public  String appNativePath = "";
	public  int curListIndex=0;
	public  void export(HSSFWorkbook tpl, int sheetIndex, ExcelBuilderConfig cfg, JSONObject obj, ExcelBuilderCallback cb) {
		...
		for(ListPropertyConfig config : cfg.getListPropertyConfig()) {
			...
			if(arr != null) {
				...
				if(config.getRowAddType()==1){
					//忽略什么也不做
					builder.prepareRows(config.getTplRow()-deledIndex, 0);
				}else{
	...
	//	电子签名人员
	private  int calcImgLocation(String value, int[] lc, int w, int h){
	 ...
	 * @return
	 */
	private  List<Map.Entry<Object, Object>> allENames;
	
	private  List<Map.Entry<Object, Object>> getLeaderImgPath(String value){
	...
	private  Map.Entry<Object, Object> getLeaderImgPathForExists(String value, List<Map.Entry<Object, Object>> lds){
			for (Map.Entry<Object, Object> e : allENames) {
				if(value.indexOf((String)e.getValue()) >= 0){
					return e;
				}
	...
	private  int[] insertImgForLeaders(HSSFWorkbook wb, String value, short r, int c) {
	    int   imgWidth   =   350; 
	    int   imgHeight   =   0;
			...
			for (int j=0; j<nvs.length; j++) {
				...
				if(j%2==0){
					...
					if(nld != null){
						...
						InputStream nf=ExcelProcess.class.getClassLoader().getResourceAsStream(filPath);
	...
	private  Object toCellValue(JSONObject obj,
		...
		if(propertyName==null || propertyName.equals("")){
			...
			return curListIndex+"";
		}else if(!obj.containsKey(propertyName)){
			return null;
		}
